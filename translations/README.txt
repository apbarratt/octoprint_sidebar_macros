Your plugin's translations will reside here. You can manage translations with
pybabel (or via Taskfile tasks if present):
...
`octoprint_sidebar_macros/translations`. When that folder exists,
an additional command becomes available:

babel_bundle --locale=<locale>
    Moves the translation for locale `<locale>` to octoprint_sidebar_macros/translations,
